...
bool GetMipLevelPixelColorARGB(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_COLOR& color)
{
    ...
    CMP_DWORD dwPitch = ((8 * miplevel.m_nWidth) + 7) / 8;
	color.asDword = 0;
	switch(textureDataType)
	{
		case TDT_RG:
		...
		default:
			color = *((CMP_COLOR*) miplevel.m_pbData + (dwPitch * nYpos) + nXpos);
			break;
	}
...
bool GetMipLevelPixelColorARGB(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_DWORD& dwColor)
{
	if(miplevel.m_pcData == NULL)
		return false;
	if(nXpos >= miplevel.m_nWidth || nXpos < 0)
	...
	{
		case TDT_R:
			dwColor= (CMP_DWORD) (*((BYTE*) miplevel.m_pbData + (dwPitch * nYpos) + nXpos));
			break;

		case TDT_RG:
			dwColor = (CMP_DWORD) (*((WORD*) miplevel.m_pbData + (dwPitch * nYpos) + nXpos));
			break;

		default:
			dwColor = *((CMP_DWORD*) miplevel.m_pbData + (dwPitch * nYpos) + nXpos);
			break;
	}
...
bool GetMipLevelPixelColorARGB(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, WORD color[4])
{
	...
	CMP_DWORD dwOffset = ((miplevel.m_nWidth * nYpos) + nXpos) * GetChannelCount(textureDataType);
...
bool GetMipLevelPixelColorARGB(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_DWORD color[4])
{
	if(miplevel.m_pcData == NULL)
		return false;
	if(nXpos >= miplevel.m_nWidth || nXpos < 0)
...
bool GetMipLevelPixelColorARGB(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, float color[4])
{
	...
	CMP_DWORD dwPitch = ((8 * miplevel.m_nWidth) + 7) / 8;
	CMP_DWORD dwColor = *((CMP_DWORD*) miplevel.m_pbData + (dwPitch * nYpos) + nXpos);
	memset(color, 0, 4 * sizeof(color[0]));
...
bool GetMipLevelPixelColorFloat32(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_COLOR& color)
{
	if(miplevel.m_pcData == NULL)
		return false;
	if(nXpos >= miplevel.m_nWidth || nXpos < 0)
...
inline uint8_t half_to_byte(const uint16_t &h)
{
	const float f=((h&0x8000)<<16) | (((h&0x7c00)+0x1C000)<<13) | ((h&0x03FF)<<13);
...
bool GetMipLevelPixelColorFloat16(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_COLOR& color)
{
	if(miplevel.m_pcData == NULL)
		return false;
	if(nXpos >= miplevel.m_nWidth || nXpos < 0)
	...
	color.asDword = 0;
	RGBA16f *pData = (RGBA16f *) (miplevel.m_pfData + (((miplevel.m_nWidth * nYpos) + nXpos) * 2));
	color.rgba[2] = half_to_byte(pData->r);
	if(textureDataType != TDT_R)
	{
		color.rgba[1] = half_to_byte(pData->g);
		if(textureDataType != TDT_RG)
		{
			color.rgba[0] = half_to_byte(pData->b);
			color.rgba[3] = half_to_byte(pData->a);
		}
	}
...
bool GetMipLevelPixelColorInt16(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_COLOR& color)
{
	if(miplevel.m_pcData == NULL)
		return false;
	if(nXpos >= miplevel.m_nWidth || nXpos < 0)
...
bool GetMipLevelPixelColorInt32(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_COLOR& color)
{
	if(miplevel.m_pcData == NULL)
		return false;
	if(nXpos >= miplevel.m_nWidth || nXpos < 0)
	...
	color.asDword = 0;
	CMP_DWORD* pData = miplevel.m_pdwData + (((miplevel.m_nWidth * nYpos) + nXpos) * GetChannelCount(textureDataType));
...
bool GetMipLevelPixelColor2101010(const MipLevel& miplevel, TextureDataType textureDataType, int nXpos, int nYpos, CMP_COLOR& color)
{
	if(miplevel.m_pcData == NULL)
		return false;
	if(nXpos >= miplevel.m_nWidth || nXpos < 0)
...
bool GetMipSetPixelColorARGB8888(const MipSet& mipset, int nMipLevel, int nFaceOrSlice, int nXpos, int nYpos, CMP_COLOR& color)
{
...
bool GetMipSetPixelColorARGB32(const MipSet& mipset, int nMipLevel, int nFaceOrSlice, int nXpos, int nYpos, CMP_DWORD color[4])
{
	if(nMipLevel >= mipset.m_nMipLevels)
		return false;
